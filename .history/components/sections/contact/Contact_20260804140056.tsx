            ...
            <div className="grid grid-cols-2 gap-px bg-border/50 border-t border-border/50">
              {socialLinks.map((item, i) => {
                ...
                return (
                  ...
                  >
                    <Icon className="size-4 text-primary shrink-0 transition-transform group-hover:scale-110" />
                    <span className="text-sm font-semibold text-foreground group-hover:text-primary transition-colors">
                      {labelFix(item.label, Icon)}
                    </span>
                  </a>
                )
              })}
...
/** Helper to render proper icon component structure for simple-icons vs standard JSX components */
function labelFix(label: string, Icon: any) {
  // If simple-icons object is passed directly instead of a JSX component
  if (typeof Icon === "object" && Icon.path) {
    return (
      <span className="flex items-center gap-2">
        <svg role="img" viewBox="0 0 24 24" className="size-4 fill-primary shrink-0">
          <path d={Icon.path} />
        </svg>
        {label}
      </span>
